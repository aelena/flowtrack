  ...
  <div class="project-page">
      ...
      <header class="project-header">
          ...
          <div>
            ...
            {#if project.status && project.status !== 'active'}
              <span class="status-badge {project.status}"
                >{project.status === 'on_hold' ? 'On Hold' : 'Deprecated'}</span
              >
            {/if}
          </div>
        ...
        <div class="stars">
          {#each [1, 2, 3, 4, 5] as n}
            <button
              type="button"
              class:active={project.star_rating >= n}
              aria-label={`Rate ${n} of 5`}
              aria-pressed={project.star_rating === n}
              on:click={() => setRating(n)}>★</button
            >
          {/each}
        </div>
        ...
        <div class="progress-row">
          <div class="progress-item">
            <span class="progress-label"
              >{t('completion', $language)}: {project.task_completion}%</span
            >
        ...
        <div class="mode-tabs">
          <button
            class:active={mode === 'overview' && !editing}
            on:click={() => {
              editing = false;
              mode = 'overview';
            }}>Overview</button
          >
          <button
            class:active={mode === 'write' && !editing}
            on:click={() => {
              editing = false;
              mode = 'write';
            }}>{t('writeMode', $language)}</button
          >
          <button
            class:active={mode === 'chat' && !editing}
            on:click={() => {
              editing = false;
              mode = 'chat';
      ...
      {#if editing}
        <div class="edit-form">
          ...
          <label
            ...
            <textarea bind:value={editData.description} rows="3"></textarea></label
          >
          <label
            >{t('vision', $language)}
            <textarea bind:value={editData.vision} rows="2"></textarea></label
          >
          <label
            >{t('goal', $language)} <textarea bind:value={editData.goal} rows="2"></textarea></label
          >
          <label
            >Completion Criteria <textarea bind:value={editData.completion_criteria} rows="2"
            ></textarea></label
          >
          <label
            >Abandonment Criteria <textarea bind:value={editData.abandonment_criteria} rows="2"
            ></textarea></label
          >
          <label
          ...
          <label
            >Subjective Completion (%) <input
              type="number"
              min="0"
              max="100"
              bind:value={editData.subjective_completion}
            /></label
          >
        ...
        <div class="project-details">
          ...
          {#if project.github_repo}
            <section>
              ...
              <a href={project.github_repo} target="_blank">{project.github_repo}</a>
            </section>
          {/if}
          {#if project.website}
            <section>
          ...
          {#if project.collaborators?.length}
            <section>
              ...
              {project.collaborators
                .map((c) => (typeof c === 'string' ? c : c.name || JSON.stringify(c)))
                .join(', ')}
            </section>
          {/if}
    ...
    <aside class="file-panel">
      ...
      <div class="file-tree">
        {#if files.length === 0}
          <p class="file-empty">
            No files yet. Upload PDFs, DOCX, or .md files to associate with this project.
          </p>
        {:else}
            ...
            <div class="file-folder">
              {#if folderName}
                <button class="file-folder-header" on:click={() => toggleFileFolder(folderName)}>
                  ...
                  <svg viewBox="0 0 16 16" class="folder-icon"
                    ><path d="M2 3h4l2 2h6v8H2z" fill="currentColor" opacity="0.6" /></svg
                  >
...
<style>
  ...
  .file-tree-item:hover .file-tree-action {
    opacity: 1;
  }
